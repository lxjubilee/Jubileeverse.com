/**
 * AutomationNodeTree.tsx — Recursive collapsible tree for automation navigation nodes
 *
 * Mirrors PromptNodeTree.tsx exactly, with one key difference:
 * context menu is conditional on isAdmin prop.
 */

import * as React from 'react'
import * as ContextMenu from '@radix-ui/react-context-menu'
import {
  useCreateAutomationTreeNode,
  useUpdateAutomationTreeNode,
  useDeleteAutomationTreeNode,
} from '../../hooks/useAutomationTree'
import type { AutomationNavNode } from '../../lib/api'

interface AutomationNodeTreeProps {
  node: AutomationNavNode
  depth: number
  isAdmin: boolean
  searchQuery?: string
...
export function AutomationNodeTree({
  node,
  depth,
  isAdmin,
  searchQuery,
  selectedNodeId,
  onSelect,
}: AutomationNodeTreeProps) {
  const [expanded, setExpanded] = React.useState(depth < 1)
  const children = node.children ?? []

  const createNode = useCreateAutomationTreeNode()
  const updateNode = useUpdateAutomationTreeNode()
  const deleteNode = useDeleteAutomationTreeNode()
  ...
  return (
    <div>
      {isAdmin ? (
        <ContextMenu.Root>
          <ContextMenu.Trigger asChild>{nodeRow}</ContextMenu.Trigger>
          <ContextMenu.Portal>
...
function nodeOrDescendantMatches(node: AutomationNavNode, query: string): boolean {
