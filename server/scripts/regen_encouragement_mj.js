#!/usr/bin/env node
/**
 * Regenerate all Encouragement category images using the Jubilee MidJourney model.
 *
 * Uses:
 *   - Mistral-7B-Instruct at localhost:9001 for cinematic prompt generation
 *   - InspireCortex /v1/midjourney/generate (synchronous, ~45-90s per image)
 *   - Juggernaut XL v9 + ESRGAN 4x upscaling + Mistral prompt enhancement
 *
 * Run: INSPIRECORTEX_JWT=<token> node scripts/regen_encouragement_mj.js
 */
'use strict';
...
const ENCOURAGEMENT_CATEGORY_ID = 64590;
const CATEGORY_SLUG  = 'encouragement';
...
const MISTRAL_URL    = 'http://localhost:9001/v1/chat/completions';
...
1. Subject, action, emotional expression at a moment of joy or breakthrough
2. Exact setting, time of day, lighting
...
function buildFallbackPrompt(article, gender, age, eth, viewpoint) {
    ...
    return `A ${age} ${eth} ${gender} fully clothed in modest attire, experiencing a moment of peaceful joy and spiritual renewal in ${setting}. ${viewpoint}, expression radiates gratitude and quiet strength as warm golden-hour light bathes the scene. Shot on 85mm f/2.0, shallow depth of field, rich warm amber and gold tones. Photorealistic, cinematic, 8K DSLR quality. Negative: nsfw, nude, naked, bare skin, topless, explicit, sexual content, revealing clothing, underwear, cleavage, ugly, deformed, noisy, blurry, bad anatomy, extra fingers, missing fingers, fused digits, distorted hands, watermarks, text, dark mood, despair, crying, hopeless, studio lights, overexposed, low quality`;
}
...
function callMidjourneyGenerate(prompt) {
    return new Promise((resolve, reject) => {
        ...
        const opts = {
            ...
            timeout:  300000,  // 300s timeout (MidJourney generates 4 variations ~87-130s)
        };
...
async function main() {
    log('='.repeat(70));
    log('JubileeVerse — Encouragement Images via Jubilee MidJourney Model');
    ...
    // 1. Fetch articles
    log('\n[1] Fetching Encouragement articles...');
    ...
    log('    (~40s per article, total estimate: ' + Math.round(jobs.length * 40 / 60) + ' minutes)\n');
