/**
 * Faith Builders Category Image Regeneration Pipeline
 *
 * Uses:
 ...
 *
 * Flow:
 *   1. Fetch all Faith Builders articles from jubileeverse DB
 ...
 *
 * Run: node scripts/generate_faith_builders_images.js
 */
...
const FAITH_BUILDERS_CATEGORY_ID = 64592;   // L2 — Faith Builders under jubileeinspire.com (64148)
...
async function main() {
    log('='.repeat(70));
    log('JubileeVerse — Faith Builders Category Image Regeneration');
    ...
    // 1. Fetch articles
    log('\n[1/7] Fetching Faith Builders articles...');
    ...
    log('\n[3/7] Archiving existing Faith Builders images...');
    ...
    for (let batchStart = 0; batchStart < promptBatch.length; batchStart += BATCH_SIZE) {
        ...
        const genResult = spawnSync(
            ...
            { encoding: 'utf8', timeout: 4 * 60 * 60 * 1000 }
        );
            ...
            for (const result of results) {
                ...
                if (result.status === 'generated') {
                    ...
                    try {
                        ...
                        // 7. Update DB
                        const relPath = `/images/jubileeinspire.com/${CATEGORY_SLUG}/${result.id}.jpg`;
    ...
    const reportPath = path.join(__dirname, '..', 'logs', `faith_builders_regen_${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
