...
/**
 * The JubileeInspire family rail — the fixed left sidebar carried by every site
 * in the network, ported from the copy kJubilee.com serves at
 * /css/inspire-rail.css. Styling lives in src/styles/inspire-rail.css, which
 * documents what this port changed and why.
 *
 * Collapsed it is a 52px column of icons; the NAVIGATION row expands it to
 * 280px and the chevron closes it again. Below 1024px it goes off-canvas and
 * the fixed hamburger is the only way in.
 */

interface RailLink {
  label: string;
  href: string;
  /** Set on the one entry that is this site, so the row paints as current. */
  current?: boolean;
  /** Every icon but the hamburger is a Material symbol on this viewBox. */
  viewBox?: string;
  path: string;
}
...
/* The family, in the order the rail carries it on every other site. */
const LINKS: RailLink[] = [
  ...
  {
    // This site. Internal link, and the one row that paints as current.
    label: 'Jubilee News',
    href: '/',
    current: true,
...
export default function InspireRail() {
  ...
  // Escape closes it from anywhere — the only way out of the mobile drawer for
  // a keyboard user, since the backdrop is a pointer target.
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <>
      ...
      {/* Pointer-only: Escape above is the keyboard equivalent, so this carries
          no role and no tab stop rather than pretending to be a button. */}
      {open ? <div className="jir-backdrop" onClick={close} /> : null}
      ...
      <nav className={`jir jir-rail${open ? ' is-open' : ''}`} aria-label="JubileeInspire">
        ...
        {LINKS.map((link) => {
          const className = `jir-item${link.current ? ' is-active' : ''}`;
          const icon = (
            <>
              <svg viewBox={link.viewBox ?? '0 -960 960 960'} aria-hidden="true">
                <path d={link.path} />
              </svg>
              <span className="jir-label">{link.label}</span>
            </>
          );

          // The one internal destination routes through Next; the rest leave
          // for sister sites and are plain anchors.
          return link.current ? (
            <Link
              key={link.label}
              className={className}
        ...
        {/* tabIndex -1: the avatar below is the same link to the same place, and
            two adjacent tab stops onto one destination is noise for a screen
            reader walking the rail. */}
        ...
        <a
          ...
          href="https://www.jubileeinspire.com/"
          title="JubileeInspire.com"
          rel="noopener"
        >
          {/*
            Next's own public/, deliberately NOT /images — that path rewrites to
            Express, and the header logo above learned the hard way that the
            originals there are megabyte PNGs served through the dev proxy.
          */}
          <img src="/brand/brand-logo.png" alt="JubileeInspire" width={36} height={36} />
        </a>
      </nav>
    </>
  );
